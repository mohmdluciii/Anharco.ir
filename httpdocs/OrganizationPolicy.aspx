...
<head id="Head1" runat="server">
    <title>Organizational Policy - ANHAR Company</title>
     ...
     <asp:AutoCompleteExtender ID="AutoCompleteExtender1" runat="server" TargetControlID="txt_Search" ServiceMethod="Buscar" ServicePath="AutoCompletar.asmx" MinimumPrefixLength="2" CompletionInterval="1000" EnableCaching="true" CompletionSetCount="12"   CompletionListCssClass="autocomplete_completionListElement" CompletionListItemCssClass="autocomplete_listItem" CompletionListHighlightedItemCssClass="autocomplete_highlightedListItem"></asp:AutoCompleteExtender>  
        ...
        <div class ="backMobileMenu" id="mobileMenu4"  style ="display:none;">
           ...
           <div class ="mainScreenLayer">    
            ...
            <div class ="MessageCEO_HeaderTextContainer"><h1 class="<%=SiteStudioStore.AlignClass("copy_OrganizationPolicy_h1")%>"><%=SiteStudioStore.Copy("copy_OrganizationPolicy_h1", "Organizational Policy")%></h1> </div>
            <div class ="MessageCEO_topTextContainer">
                <p class="<%=SiteStudioStore.AlignClass("copy_OrganizationPolicy_p1")%>"><%=SiteStudioStore.Copy("copy_OrganizationPolicy_p1", "Policy is a strategic document that is determined by the top management of the organization. While referring to quality commitments, applicable requirements and continuous improvement according to the organization’s business environment, the quality policy is in line with the vision and mission of the organization and provides a framework for determining quality objectives. The Health, Safety and Environment (HSE) policy also provides a framework for setting HSE-related goals while committing to complying with legal requirements, preventing pollution, eliminating hazards, reducing health and safety risks, continuous improvement and workers’ participation.")%></p>
            </div>
